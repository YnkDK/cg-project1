...
#include "MbC2_CH.h"
inline double ccw3(point *p1, point *p2, point *p3) {
             double left  = (p1->x - p3->x)*(p2->y - p3->y);
	      double right = (p1->y - p3->y)*(p2->x - p3->x);
	      return left - right;
}

void MbC2_CH::upperHull(std::deque<point>& points,	//< Points on which the CH should be computed
	 ...
	 */
	upperHull(Pl, Pl.size(), result);
	upperHull(Pr, Pr.size(), result);
}

void MbC2_CH::prune(std::deque< point >& points, std::deque<point>& lowerHullPoints, std::deque<point> &upperHullPoints)
{
	...
	for(size_t i = 0; i < size; i++){
	    double tmp = ccw3(&min, &max, &points[i]);
		if(tmp >= 0)
...
inline void MbC2_CH::convexHull(std::deque<point>& points, const int numPoints, std::deque<point> &result) {
	std::deque<point> lowerHullPoints, upperHullPoints;
	// Compute the upper hull
	prune(points, lowerHullPoints, upperHullPoints);
	
	upperHull(upperHullPoints, upperHullPoints.size(), result);
	// Compute the 'upper' hull in the mirrored set of points
	result.pop_front();
	upperHull(lowerHullPoints, lowerHullPoints.size(), result);
	result.pop_back();
	// Make it clock-wise
